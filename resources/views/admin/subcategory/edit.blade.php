...
<li class="breadcrumb-item fw-bolder"><a class="nav-link d-inline" href="{{route('SubCategory-dashboard.index')}}">الأقسام الفرعية</a></li>
    <li class="breadcrumb-item fw-bolder active " aria-current="page">تعديل قسم {{$subcategory->title}}</li>
@endsection
...
<div class="card mx-1 mx-md-3 mt-3 mb-5 border-0 rounded-5 shadow-sm">
  ...
  <form class="form  mx-2 rounded-5" action="{{route('SubCategory-dashboard.update',$subcategory -> id)}}" method="POST" enctype="multipart/form-data">
  @csrf
  @method('PUT')

  <input name="id" value="{{$subcategory -> id}}" type="hidden">
  
  <div class="fw-bolder m-2 text-center p-3 border-bottom fs-4 text-dark border-2">تعديل القسم الفرعي</div>
    ...
    <div class="px-4">
        ...
        <div class="  mb-3">
            <div class=" my-3">
                ...
                <input id="title" type="text" name="title" class="form-control  rounded-4"   value="{{$subcategory -> title}}" >
            </div>
            @error('title')
        ...
        <div class="  mb-3">
            <div class=" my-3">
                ...
                <input id="slug" type="text" name="slug" class="form-control  rounded-4"   value="{{$subcategory -> slug}}" >
            </div>
            @error('slug')
                <div class='bg-danger text-white m-0 p-2 fw-bolder w-100'>{{ $message }}</div>
            @enderror
        ...
        <div class="  mb-3">
            <div class=" my-3">
                ...
                <input id="icon" type="text" name="icon" class="form-control  rounded-4"   value="{{$subcategory -> icon}}" >
            </div>
            @error('icon')
                <div class='bg-danger text-white m-0 p-2 fw-bolder w-100'>{{ $message }}</div>
            @enderror
        ...
        <div class="  mb-3">
            <div class="mb-3 ">
                ...
                <textarea id="description" type="textarea" name="description" class="fw-bolder form-control border border-dark border-2 p-3 rounded-4" rows="5">{{$subcategory -> description}}</textarea>
            </div>
            @error('description')
                <div class='bg-danger text-white m-0 p-2 fw-bolder w-100'>{{ $message }}</div>
            @enderror
...
<!-- 
    ...
    <form class="form mx-2 rounded-3" action="{{route('SubCategory-dashboard.update',$subcategory -> id)}}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <h5 class="font-weight-bolder m-2 text-center p-3 border-bottom border-2">تعديل القسم الفرعي</h5>
        ...
        <div class="row  px-4">
            ...
            <div class="col-md-10  mb-5">
                ...
                <div class="input-group input-group-outline  my-3">
                    <input type="text" name="title" value="{{$subcategory -> title}}" class="font-weight-bolder form-control">
                </div>
                @error('title')
            ...
            <div class="col-md-10  mb-5">
                <h6 class="font-weight-bolder m-2 text-end">الايقونة <i class="{{$subcategory -> icon}}"></i></h6>
                <div class="input-group input-group-outline  my-3">
                    <input type="text" name="icon" value="{{$subcategory -> icon}}" class="font-weight-bolder form-control">
                </div>
                @error('icon')
                    <p class='w-100 bg-danger alert text-light m-0 p-2'>
                        <strong class="font-weight-bolder ">{{ $message }}</strong>
...
<script>
    ...
    let old_color = "{{$subcategory -> color}}";
