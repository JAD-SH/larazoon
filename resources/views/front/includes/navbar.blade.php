    ...
    <div class="container-fluid d-block d-md-flex justify-content-center justify-content-md-between text-nowrap">
      ...
      <ul class="navbar-nav p-0  flex-row justify-content-around">
        ...
        <li class="d-flex d-md-none  nav-item ">
          <a class="p-0 nav-link fw-bolder text-black d-flex align-items-center" href="{{route('Course.index')}}">
            <i class="fa-solid fs-6 fa-house-chimney"></i>
          </a>
        </li>
        <li class="d-flex d-md-none  nav-item ">
          <a class="p-0 nav-link fw-bolder text-black d-flex align-items-center" href="{{route('Course.index')}}">
      ...
      <ul class="navbar-nav p-0 flex-row d-none d-md-flex">
        <li class="nav-item d-flex align-items-center d-block d-md-none">
          ...
          <div class="dropdown position-relative">
            <a class=" position-relative" type="button" data-bs-toggle="dropdown" aria-expanded="false">
              ...
              || Auth::user()->supports()->where('watch','0')->count() > 0)
                <span class="position-absolute top-0  start-100 translate-middle badge rounded-pill bg-danger" style="font-size:10px !important;">
            ...
            <ul class="dropdown-menu dropdown-menu-dark rounded-5 p-2 position-absolute shadow-sm">
              <li>
                <a href="{{route('profile')}}" class=" btn dropdown-item rounded-5 ">
                إجابات اسئلتك
              ...
              <li>
                <a  href="{{route('User.supporter-archive',Auth::user()->username)}}" class=" btn dropdown-item rounded-5 ">
                  اشعارات سجل الدعم
                  <span class=" translate-middle rounded-pill bg-danger bg-gradient-danger text-white p-1 px-2 mx-1" style="font-size:10px !important;">
                    {{Auth::user()->supports()->where('watch','0')->count()}}
                  </span>
              ...
              <li>
                <a href="{{route('User.site-media-page')}}" class=" btn dropdown-item rounded-5 ">
                ردود الادارة
                <span class=" translate-middle rounded-pill bg-danger bg-gradient-danger text-white p-1 px-2 mx-1" style="font-size:10px !important;">
                {{Auth::user()->notification()->whereHas('notificationreply')->get()->pluck('notificationreply')->flatten()->where('watch','0')->count()}}
                </span>
              ...
              @else
                <li>
                  <a  class=" btn dropdown-item rounded-5 ">
                  رد على تعليقك
