...
<li class="breadcrumb-item fw-bolder"><a class="nav-link d-inline" href="{{route('ArticleLibrary-dashboard.index')}}">قسم مكتبة المقالات</a></li>
    <li class="breadcrumb-item fw-bolder active " aria-current="page">سلة المقالات المحذوفة </li>
...
@section('content')
    ...
    <div class="card mx-1 mx-md-3  border-0 rounded-5  shadow-sm mt-3 mb-5 overflow-hidden">
        <h5 class="fw-bolder m-2 fs-5 text-dark text-center p-3">سلة المقالات المحذوفة</h5>
            ...
            <table width="100%" class="table-striped col-12 w-100 table align-items-center mb-0 mx-2 rounded-4 border  border-top-0" cellspacing="0">
                ...
                <tbody class="table-group-divider text-nowrap align-middle">
                    ...
                    <tr id="item-{{$article->id}}">
                        ...
                        <td class=" text-center">
                            <div class="m-3">
                                <span class="mb-0 fw-bolder  text-uppercase fs-6 text-dark">{{$article->articlelibrary->title}}</span>
                            </div>
                        </td>
                        ...
                        <td class=" text-center">
                            <div class="m-3">
                                <span class="mb-0 fw-bolder">{{$article -> deleted_at->diffForHumans()}}</span>
                            </div>
                        </td>
                        
                        <td class=" text-center">
                            <div class="m-3">
                                <form class="d-inline delete-form-notification" action="{{route('ArticleLibrary-dashboard.restore',$article -> id)}}" method="POST">
                                    @csrf
                                    <button type="button" 
    ...
    @else
    <div class="fs-4 text-center p-5 font-weight-bolder">لا يوجد مقالات في سلة المهملات</div>
