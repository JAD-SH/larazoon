...
<li class="breadcrumb-item fw-bolder"><a class="nav-link d-inline" href="{{route('Plan-dashboard.index')}}">قسم خطط التعلم</a></li>
    <li class="breadcrumb-item fw-bolder active " aria-current="page">متابعي الخطة - {{$users[0]->plan->title}}</li>
@endsection
                ...
                <tr>
                    ...
                    <td  class=" text-center" >
                        <div class="m-3">
                            ...
                            <form class="d-inline" action_data="{{route('User-dashboard.status',$user -> id)}}" method_data="POST">
                                @csrf
                            ...
                            <button type="button" action_data="{{route('User-dashboard.destroy',$user -> id)}}"
...
@else
    @section('content')
        <div class="fs-4 text-center p-5 font-weight-bolder">لا احد يتابع الخطة بعد</div>
    @endsection
@endif
